...
def send_admin_message(session_id: int, content: str, db: Session = Depends(get_db), current=Depends(require_role("staff"))):
    session = db.query(ConversationSession).filter(ConversationSession.id == session_id).first()
    if not session:
        raise HTTPException(status_code=404, detail="session not found")
    ...
    if content.lower().startswith("quote:"):
        message_type = "quote"
        content_body = content[len("quote:"):].strip()
    else:
        ...
        if message_type == "quote":
            ...
            if content_body.lower().startswith("confirm") or content_body.lower().startswith("confirm:"):
                ...
                if customer and session.current_product and session.current_quantity:
                    # find product
                    prod = db.query(Product).filter(Product.wood_type.ilike(f"%{session.current_product}%"), Product.size.ilike(f"%{session.current_size}%")).first()
    ...
    try:
        ...
        import asyncio
        asyncio.create_task(publish_admin_notification(1, {"type": "new_admin_message", "session_id": session.id, "message": content_body}))
    except Exception:
        pass
